...
class UserService:
    @classmethod
    def has_user_by_telegram_id(cls, db: Session, telegram_id: int) -> bool:
    ...
    def create_user(cls, db: Session, user: schemas.UserCreate):
        ...
        if cls.has_user_by_telegram_id(db, telegram_id=user.telegram_id):
...
class CohortService:
    @classmethod
    def get_cohorts(cls, db: Session, skip: int = 0, limit: int = 100):
        return db.query(models.Cohort).offset(skip).limit(limit).all()

    @classmethod
    def has_cohort_by_uuid(cls, db: Session, uuid: UUID) -> bool:
    ...
    @classmethod
    def create_cohort(cls, db: Session, cohort: schemas.CohortCreate):
        """Создать когорту в БД, если еще нет когорты с таким uuid."""
        if cls.has_cohort_by_uuid(db, cohort.notion_db_id):
